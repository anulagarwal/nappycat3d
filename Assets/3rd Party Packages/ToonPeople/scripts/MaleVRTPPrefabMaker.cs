...
public class MaleVRTPPrefabMaker : MonoBehaviour
{
...

    {
        ...
        for (int forAUX = 0; forAUX < 2; forAUX++) GOchest[forAUX] = (GetComponent<Transform>().GetChild(forAUX + 15).gameObject);
        for (int forAUX = 0; forAUX < 2; forAUX++) GOchest[forAUX + 2] = (GetComponent<Transform>().GetChild(forAUX + 19).gameObject);
            ...
            for (int forAUX2 = 0; forAUX2 < materialcount; forAUX2++)
                for (int forAUX3 = 0; forAUX3 < 4; forAUX3++)
                    for (int forAUX4 = 1; forAUX4 < 5; forAUX4++)
                    {
                        if (AUXmaterials[forAUX2].name == allskins[forAUX3] + forAUX4)
                        {
                            AUXmaterials[forAUX2] = MATSkins[0] as Material;
